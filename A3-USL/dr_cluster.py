...
def pca_clust(X, y, random_seed, filename, classifier_col, verbose=False):
    print(filename, 'PCA')
    ...
    pca = PCA(svd_solver='full', random_state=random_seed, n_components=n_com)
    pcaX = pca.fit_transform(X)
    ul_Kmeans(pcaX, y, random_seed, filename + 'kmeans', classifier_col, verbose)
    ul_EM(pcaX, y, random_seed, filename + 'EM', classifier_col, verbose)


def ica_clust(X, y, random_seed, filename, classifier_col, verbose=False):
    print(filename, 'ICA')
    if filename == 'Mobile_Prices':
        n_com = 6
    else:
    ...
    ica = FastICA(random_state=random_seed, n_components=n_com)
    icaX = ica.fit_transform(X)
    ul_Kmeans(icaX, y, random_seed, filename + 'kmeans', classifier_col, verbose)
    ul_EM(icaX, y, random_seed, filename + 'EM', classifier_col, verbose)
...
def rp_clust(X, y, random_seed, filename, classifier_col, verbose=False):
    ...
    ul_Kmeans(rpX, y, random_seed, filename + 'kmeans', classifier_col, verbose)
    ul_EM(rpX, y, random_seed, filename + 'EM', classifier_col, verbose)


def lle_clust(X, y, random_seed, filename, classifier_col, verbose=False):
    print(filename, 'LLE')
    if filename == 'Mobile_Prices':
        n_com = 4
    else:
    ...
    lle = LocallyLinearEmbedding(n_neighbors=10, n_components=n_com, random_state=random_seed, n_jobs=-1)
    lleX = lle.fit_transform(X)
    #ul_Kmeans(lleX, y, random_seed, filename + 'kmeans', classifier_col, verbose)
    ul_EM(lleX, y, random_seed, filename + 'EM', classifier_col, verbose)
